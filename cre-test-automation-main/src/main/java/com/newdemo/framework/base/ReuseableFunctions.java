...
public class ReuseableFunctions {
    ...
    public static Utilites objUtilitess = new Utilites();
...
//static Screen s = new Screen();
    ...
    public synchronized Boolean launchURL(String strURL) throws Exception {
        try {
            ...
            message = ": " + " " + "Unable to launch URL=>" + strURL + " in browser";
            return false;
        }
    }
    ...
    public synchronized Boolean clickObject(WebElement objWebElement, String strObjectName) throws Exception {
        try {
            ...
            strErrorMsg = objUtilitess.getExceptionNDisplay(objException, true);
            message = ": " + strObjectName + " couldn't be clicked.";
    ...
    public synchronized boolean waitForElement(WebElement objWebElement) throws Exception {
        ...
        try {
            ...
            strErrorMsg = objUtilitess.getExceptionNDisplay(objException, true);
            String locator = getLocatorStringFromBy(getLocatorFromElement(objWebElement));
    ...
    public synchronized void waitForElementWithRefresh(WebElement element) throws Exception {
        boolean found = false;
        for (int i = 0; i < 2; i++) {
            found = waitForElement(element);
            if (found) {
                break;
            } else {
    ...
    public String getLocatorStringFromBy(By findBy) throws Exception {
        String locatorType = "xpathExpression";
        if ((By) findBy instanceof ByCssSelector) {
            locatorType = "selector";
        }

        return (String) FieldUtils.readField((By) findBy, locatorType, true);
    }
    ...
    public static void renameDownloadedfiles(String portfolio, String env, File difference) throws ScriptException, IOException, InterruptedException {
        ...
        for (int i = 0; i < listFiles.length; i++) {
            ...
            if (listFiles[i].isFile()) {
                ...
                } else if (fileName.startsWith("portfolio_calculator_") && fileName.endsWith(".xlsx")) {
                    File file = new File(folderName + fileName);
                    System.out.println(file);
    ...
    public static void deleteoldDownloadedfiles(String portfolio, String env, File difference) throws ScriptException, IOException, InterruptedException {
        ...
        for (int i = 0; i < listFiles.length; i++) {
            ...
            if (listFiles[i].isFile()) {
                ...
                } else if (fileName.startsWith(portfolio) && fileName.endsWith(".xlsx")) {
                    File file = new File(folderName + portfolio + env + ".xlsx");
                    file.delete();
                    System.err.println(file + " " + "Old file deleted 1");
                } else if (fileName.startsWith("difference" + portfolio) && fileName.endsWith(".xlsx")) {

                    difference.delete();
                    System.err.println(portfolio + "Old differenes file deleted 2 ");
                } else if (fileName.startsWith("difference" + portfolio + "-LBL") && fileName.endsWith(".xlsx")) {
                    difference.delete();
    ...
    public long printPageLoadTime() {
        ...
        long performanceCalc = domComplete - navigationStart;

        System.out.println("Page Load Time: " + performanceCalc);
        return performanceCalc;
    }

    public long printNGPageLoadTime() {
        long navigationStart = (long) jsexecutor.executeScript("return window.performance.timing.navigationStart");
        long finish = (long) jsexecutor.executeScript("return Date.now()");
        long performanceCalc = finish - navigationStart;
    ...
    public void runAssessibilityfunction() throws Exception {
        List<String> tags = Arrays.asList("wcag2aa");
        ...
        String pagename = name.replaceAll(" ", "_");
        File directory = new File(System.getProperty("user.dir") + "/AccessibilityAuditfiles/");
        if (!directory.exists()) {
            directory.mkdir();
        }
        
        AxeReporter.writeResultsToJsonFile(System.getProperty("user.dir") + "/AccessibilityAuditfiles/" + pagename, results);
        JSONObject jelement = (JSONObject) parser.parse(new FileReader(System.getProperty("user.dir") + "/AccessibilityAuditfiles/" + pagename + ".json"));
        String prettyJson = gson.toJson(jelement);
        AxeReporter.writeResultsToTextFile(System.getProperty("user.dir") + "/AccessibilityAuditfiles/" + pagename, prettyJson);
